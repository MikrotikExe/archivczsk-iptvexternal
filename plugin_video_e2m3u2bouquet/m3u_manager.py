...
class M3URefreshManager(object):
	...
	def inject_epg_only(self):
		...
		bouquet_dir = self._str('m3u_bouquet_dir') or DEFAULT_BOUQUET_DIR
		tv_bouquet = os.path.join(
			bouquet_dir,
			'userbouquet.{}.tv'.format(M3U_BOUQUET_PREFIX)
		)
		radio_bouquet = os.path.join(
			bouquet_dir,
			'userbouquet.{}.radio'.format(M3U_BOUQUET_PREFIX)
		)
		url_to_sref = build_url_to_sref_from_bouquet(tv_bouquet)
		if os.path.isfile(radio_bouquet):
			url_to_sref.update(build_url_to_sref_from_bouquet(radio_bouquet))
		if not url_to_sref:
		...
		self.log('[M3U-mgr] inject_epg_only: loaded %d url→sref mappings '
		         'from existing bouquet(s)' % len(url_to_sref))
